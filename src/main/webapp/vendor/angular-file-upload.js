...
module
    ...
    .factory('FileUploader', ['fileUploaderOptions', '$rootScope', '$http', '$window', '$compile',
        function(fileUploaderOptions, $rootScope, $http, $window, $compile) {
            ...
            /**
             * Adds items to the queue (soma o tamanho do arquivo no total (size))
             ...
             **********************/
            /**
             * Returns the total progress (calcula o tamanho total enviado de arquivos (sizeProgress) e a valocidade de upload)
             * @param {Number} [value]
             * @returns {Number}
             * @private
             */
            FileUploader.prototype._getTotalProgress = function(value) {
                ...
                var progress = Math.round(uploaded * ratio + current);

                var sProgress = 0;
                if((this.size > 0 && current > 0) || notUploaded == 0){
                    sProgress = Math.round((this.size / 100) * progress);
                }else{
                    sProgress = 0;
                }
                this.sizeProgress = sProgress;
                
                if(angular.isUndefined(this.tempAnt)){
                    this.tempAnt = Math.round(+new Date()/1000);
                    this.sizeAnt = sProgress;
                }
                
                this.tempAtual = Math.round(+new Date()/1000);
                this.sizeAtual = sProgress;
                
                var tempDiff = this.tempAtual - this.tempAnt;
                var sizeDiff = this.sizeAtual - this.sizeAnt;
                if(tempDiff > 0){
                    this.uploadVel = sizeDiff / tempDiff;
                }
